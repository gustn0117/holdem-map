    ...
    <>
      <header className="sticky top-0 z-50 glass border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-white/5 rounded-full p-1.5">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <Link href="/contact" className="gold-btn text-dark text-sm font-bold px-6 py-2.5 rounded-full shadow-lg shadow-accent/30 hover:shadow-accent/50 transition-all">
              매장 등록
            </Link>
          </div>
        ...
        {menuOpen && (
          <nav className="md:hidden glass border-t border-white/5 px-4 py-3 space-y-1">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className={`block px-4 py-3 rounded-xl text-base font-medium ${pathname === item.href ? "text-accent bg-accent/10" : "text-muted"}`} onClick={() => setMenuOpen(false)}>
                {item.label}
              </Link>
            ))}
          </nav>
