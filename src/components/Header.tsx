  ...
  return (
    <>
      <header className="sticky top-0 z-50 bg-white border-b border-border-custom">
        <div className="w-full mx-auto px-5 md:px-10 h-12 md:h-14 flex items-center justify-between relative" style={{ maxWidth: "1400px" }}>
          {/* Left: Logo */}
        ...
        <div className="border-t border-border-custom px-4 py-2 bg-white">
          ...
          <Link href="/tournament" className="shrink-0 btn-shimmer btn-bounce btn-glow text-white text-[10px] md:text-[12px] font-black py-2 md:py-2.5 px-3 md:px-4 rounded-full whitespace-nowrap flex items-center justify-center gap-1 relative overflow-hidden" style={{ minWidth: "130px" }}>
            <span className="firework-container"><span /><span /><span /></span>
            <svg className="w-3 h-3 text-yellow-300 shrink-0 drop-shadow relative z-10" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
            <span className="relative h-4 overflow-hidden drop-shadow z-10" style={{ width: "95px" }}>
              {tournerTexts.map((text, i) => (
                <span key={text} className="absolute inset-0 flex items-center justify-center transition-opacity duration-500" style={{ opacity: tournerBtnIdx === i ? 1 : 0 }}>{text}</span>
              ))}
            </span>
          </Link>
          </div>
